        ...
        CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    PSRCore.Functions.Notify(Lang:t('error.bled_out'), "error")
                    SetLaststand(false)
                    ...
                    local weaponItem = PSRCore.Shared.Weapons[killerWeapon]
                    if weaponItem then
                        weaponLabel = weaponItem.label
                        weaponName = weaponItem.name
                    end
                    TriggerServerEvent("psr-log:server:CreateLog", "death", Lang:t('logs.death_log_title', {playername = GetPlayerName(-1), playerid = GetPlayerServerId(player)}), "red", Lang:t('logs.death_log_message', {killername = killerName, playername = GetPlayerName(player), weaponlabel = weaponLabel, weaponname = weaponName}))
                    deathTime = 0
                    OnDeath()
                    DeathTimer()
                end
...
RegisterNetEvent('hospital:client:UseDoctorKit', function()
    if not isEscorting then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            TriggerServerEvent('hospital:server:UseDoctorKit', playerId)
        end
    else
        PSRCore.Functions.Notify(Lang:t('error.impossible'), 'error')
    end
end)
...
RegisterNetEvent('hospital:client:HelpPerson', function(targetId)
    local ped = PlayerPedId()
    PSRCore.Functions.Progressbar("hospital_revive", Lang:t('progress.revive'), math.random(30000, 60000), false, true, {

...
local function DoLimbAlert()
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            PSRCore.Functions.Notify(limbDamageMsg, "primary")
        end
    end
end
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        PSRCore.Functions.Notify(Lang:t('info.bleed_alert', {bleedstate = Config.BleedingStates[tonumber(isBleeding)].label}), "error")
    end
end
...
local function CheckWeaponDamage(ped)
    ...
    for k, v in pairs(PSRCore.Shared.Weapons) do
        if Citizen.InvokeNative(0xDCF06D0CDFF68424, ped, GetHashKey(k), 0) then
            detected = true
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    emsNotified = false
    PSRCore.Functions.Notify(Lang:t('info.healthy'))
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    PSRCore.Functions.Notify(Lang:t('success.wounds_healed'), 'success')
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function ()
        Wait(5)
        if isRevive then
            PSRCore.Functions.Notify(Lang:t('success.being_helped'), 'success')
...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports["psr-policejob"]:IsHandcuffed() then
...
RegisterNetEvent('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        if PSRCore.Functions.GetPlayerData().charinfo.gender == 1 then
            gender = Lang:t('info.mrs')
        end
        local charinfo = PSRCore.Functions.GetPlayerData().charinfo
        TriggerServerEvent('psr-phone:server:sendNewMail', {
...
CreateThread(function()
    for k, v in pairs(Config.Locations['stations']) do
        local DoctorBlip = N_0x554d9d53f696d002(1664425300, v.coords)
        SetBlipSprite(DoctorBlip, 3599598875, 52)
        SetBlipScale(DoctorBlip, 0.2)
        Citizen.InvokeNative(0x9CB1A1623062F402, tonumber(DoctorBlip), v.label)
    end
end)
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            exports['psr-core']:DrawText(Lang:t('text.bed_out'))
        ...
        while listen do
            if IsControlJustPressed(0, 38) then
                ...
                if variable == "checkin" then
                   TriggerEvent('psr-doctorjob:checkin')
                    listen = false
                elseif variable == "beds" then
                    TriggerEvent('psr-doctorjob:beds')
                    listen = false
                end
            end
            Wait(1)
...
RegisterNetEvent('psr-doctorjob:checkin', function()
    if doctorCount >= Config.MinimalDoctors then
        TriggerServerEvent("hospital:server:SendDoctorAlert")
    else
        TriggerEvent('animations:client:EmoteCommandStart', {"notepad"})
        PSRCore.Functions.Progressbar("hospital_checkin", Lang:t('progress.checking_in'), 2000, false, true, {
            ...
            if bedId then
                ...
                PSRCore.Functions.Notify(Lang:t('error.beds_taken'), "error")
            end
        end, function() -- Cancel
            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
...
RegisterNetEvent('psr-doctorjob:beds', function()
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Locations["checking"]) do
            exports['psr-target']:AddBoxZone("checking"..k, vector3(v.x, v.y, v.z), 3.5, 2, {
                name = "checking"..k,
                heading = -72,
                debugPoly = false,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            exports['psr-target']:AddBoxZone("beds"..k,  v.coords, 2.5, 2.3, {
                name = "beds"..k,
                heading = -20,
                debugPoly = false,
                minZ = v.coords.z - 1,
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["checking"]) do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        exports['psr-core']:DrawText(Lang:t('text.call_doc'),'left')
                        CheckInControls("checkin")
                    else
                        exports['psr-core']:DrawText(Lang:t('text.check_in'), 'left')
                        CheckInControls("checkin")
                    end
                else
                    listen = false
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            ...
            bedCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    exports['psr-core']:DrawText(Lang:t('text.lie_bed'), 'left')

...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive)
	...
	Player.Functions.RemoveMoney("bank", Config.BillCost , "respawned-at-hospital")
		exports['psr-management']:AddMoney("doctor", Config.BillCost)
		...
		for k, v in pairs(Config.Locations["jailbeds"]) do
			if not v.taken then
				...
				if Config.WipeInventoryOnRespawn then
					...
					TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.possessions_taken'), 'error')
				end
				Player.Functions.RemoveMoney("bank", Config.BillCost, "respawned-at-hospital")
					exports['psr-management']:AddMoney("doctor", Config.BillCost)
				TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
...
RegisterNetEvent('hospital:server:doctorAlert', function(text)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local players = PSRCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v.PlayerData.job.name == 'doctor' and v.PlayerData.job.onduty then
            TriggerClientEvent('hospital:client:doctorAlert', v.PlayerData.source, coords, text)
        end
    end
end)
...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	local src = source
	local Player = PSRCore.Functions.GetPlayer(src)
	local Patient = PSRCore.Functions.GetPlayer(playerId)
	if Patient then
		if Player.PlayerData.job.name =="doctor" then
			Player.Functions.RemoveItem('bandage', 1)
			TriggerClientEvent('inventory:client:ItemBox', src, PSRCore.Shared.Items['bandage'], "remove")
	...
	if Patient then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('inventory:client:ItemBox', src, PSRCore.Shared.Items['firstaid'], "remove")
				TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
			else
				TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.not_enough_money'), "error")
			end
		else
			Player.Functions.RemoveItem('firstaid', 1)
			TriggerClientEvent('inventory:client:ItemBox', src, PSRCore.Shared.Items['firstaid'], "remove")
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
    ...
    if not doctorCalled then
        ...
        for _, v in pairs(players) do
            if v.PlayerData.job.name == 'doctor' and v.PlayerData.job.onduty then
                TriggerClientEvent('PSRCore:Notify', v.PlayerData.source, Lang:t('info.dr_needed'), 'doctor')
            end
        end
        ...
        TriggerClientEvent('PSRCore:Notify', src, 'Doctor has already been notified', 'error')
    end
end)

RegisterNetEvent('hospital:server:UseDoctorKit', function(targetId)
	local src = source
	local Target = PSRCore.Functions.GetPlayer(targetId)
	if Target then
...
RegisterNetEvent('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('PSRCore:Notify', helperId, Lang:t('error.cant_help'), "error")
	end
end)

RegisterNetEvent('hospital:server:removeItem', function(item, amount)
...
PSRCore.Functions.CreateCallback('hospital:GetDoctors', function(_, cb)
	local amount = 0
    local players = PSRCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v.PlayerData.job.name == 'doctor' and v.PlayerData.job.onduty then
...
PSRCore.Functions.CreateCallback('hospital:GetPlayerStatus', function(_, cb, playerId)
	local Player = PSRCore.Functions.GetPlayer(playerId)
	local injuries = {}
...
PSRCore.Functions.CreateCallback('hospital:GetPlayerBleeding', function(source, cb)
	local src = source
...
PSRCore.Commands.Add('911e', Lang:t('info.ems_report'), {{name = 'message', help = Lang:t('info.message_sent')}}, false, function(source, args)
	local src = source
	local message
    ...
    for _, v in pairs(players) do
        if v.PlayerData.job.name == 'doctor' and v.PlayerData.job.onduty then
            TriggerClientEvent('hospital:client:doctorAlert', v.PlayerData.source, coords, message)
        end
    end
end)

PSRCore.Commands.Add("status", Lang:t('info.check_health'), {}, false, function(source, _)
	local src = source
	local Player = PSRCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "doctor" then
		TriggerClientEvent("hospital:client:CheckStatus", src)
	else
		TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.not_ems'), "error")
	end
end)

PSRCore.Commands.Add("heal", Lang:t('info.heal_player'), {}, false, function(source, _)
	local src = source
	local Player = PSRCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "doctor" then
...
PSRCore.Commands.Add("revivep", Lang:t('info.revive_player'), {}, false, function(source, _)
	local src = source
	local Player = PSRCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "doctor" then
...
PSRCore.Commands.Add("revive", Lang:t('info.revive_player_a'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = PSRCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
			TriggerClientEvent('hospital:client:Revive', Player.PlayerData.source)
		else
			TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.not_online'), "error")
		end
	else
...
PSRCore.Commands.Add("setpain", Lang:t('info.pain_level'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = PSRCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
PSRCore.Commands.Add("kill", Lang:t('info.kill'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = PSRCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
PSRCore.Commands.Add('aheal', Lang:t('info.heal_player_a'), {{name = 'id', help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = PSRCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
PSRCore.Functions.CreateUseableItem("ifak", function(source, item)
	local src = source
	local Player = PSRCore.Functions.GetPlayer(src)
...
PSRCore.Functions.CreateUseableItem("bandage", function(source, item)
	local src = source
	local Player = PSRCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
PSRCore.Functions.CreateUseableItem("morphine", function(source, item)
	local src = source
	local Player = PSRCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
PSRCore.Functions.CreateUseableItem("doctorkit", function(source, item)
	local src = source
	local Player = PSRCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
		TriggerClientEvent("hospital:client:UseDoctorKit", src)
	end
end)
